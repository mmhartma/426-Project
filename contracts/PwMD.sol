...
contract ERC721 {
    ...
    mapping(bytes32 => bytes) private _ivs;
    ...
    function getPassword(
        ...
        bytes32 url, 
        bytes32 username
    ) onlyOwner(tokenId, hashed_nft_password) public view returns (bytes memory, bytes memory) {
        bytes32 key = keccak256(abi.encodePacked(tokenId, url, username));
        bytes memory pass = _passwords[key];
        
        require(pass.length > 0, "Password does not exist");

        bytes memory iv = _ivs[key];

        require(iv.length > 0, "iv does not exist");
